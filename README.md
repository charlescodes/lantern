...
Start with the [documentation index](./docs/README.md). It separates mutable [soft specifications](./docs/soft-specs/README.md) from the durable [platform contract](./docs/platform.md), chronological milestone contracts, and regression notes. Release [`0.9.1`](./docs/milestones/0.9.1-authoring-and-vertical-traversal.md) is the current application checkpoint; [`0.9.0`](./docs/milestones/0.9.0-fireball-investigation-ai.md) remains the frozen Fireball-investigation boundary. The [M1A.1–M1A.4 authoring kit](./docs/notes/map-authoring-foundation.md) emits authoring-map v5 with stable layers, semantic history, holes, and connectors. [M1B.1–M1B.3](./docs/notes/generic-vertical-bodies-and-elevator.md) adds continuous gameplay Y, per-body support/layer state, elevators, multi-floor falling, committed jumps, and grounded pressure plates under snapshot/recording schema v12. Authoring-map v4/v3/v2/v1, legacy scenario v3, and map v1 remain loadable; scenario v3 remains the recording compatibility projection, Fireball definition v1 is unchanged, and schemas v2-v11 retain frozen behavior.
