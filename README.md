...
Snapshots, runtime metrics, and command recordings use schema v22. Scenario JSON uses v3 and current recordings retain an initial authoring-map v7 document alongside its frozen scenario-v3 compatibility projection. Schema v22 records `banked-sentry-obelisk-encounters-v3`: an ineligible obelisk banks at most one due spawn, releases that one immediately when the same-floor player becomes visible and enters range, then schedules later enemies from that release tick up to its authored living cap. Schema v21 preserves `sentry-obelisk-encounters-v2`, including its historical overdue catch-up cadence and the same range/visibility gate. Schema v20 preserves unconditional independent obelisk encounters; schema v19 records each enemy's floor-aware home identity and return routing; schema v18 records height-aware projectile contacts. Schema v17 records `observed-hole-pursuit-v1`: pursuit-capable enemies may follow a directly witnessed player drop through a live hole, without learning an unseen landing position; schemas v2-v16 retain their exact prior behavior. Schema v16 records `enemy-archetypes-v1` and the encounter's `wizard` or `urchin` selection; schemas v2-v15 remain wizard-only. Schema v15 records the inert `authored-navigation-topology-v1` profile and bounded capacities; schemas v2-v14 preserve their prior AI behavior with topology disabled. Schema v14 records `breakaway-floor-v1`: an 18-tick latched floor-to-hole transition that reuses ordinary aperture/falling authority. Schema v13 records the `elevator-projectile-collision-v1` profile: Fireballs collide with a stopped deck on either endpoint and the lower-floor piston/shaft while it is away from its lower stop. Schema v12 retains the committed jump edge and its pre-elevator-projectile collision behavior. Player snapshots include movement, vertical/support, layer, aperture, and jump state; enemies and their bodies retain archetype, physical identity, and live transform state. The one-tick sound queue and newest 32 of a separate 128-entry diagnostic ring expose stable source/event identity, origin, radius, and bounded occupancy/drop/hearing counters. The compact `spells` table still includes only current and referenced immutable revisions; effects retain spell code, definition revision, effect ID, and effect seed.

A live v22 recording stores `obeliskEncounterProfile: "banked-sentry-obelisk-encounters-v3"`, `enemyHomeProfile: "floor-aware-enemy-home-v1"`, `holePursuitProfile: "observed-hole-pursuit-v1"`, `enemyArchetypeProfile: "enemy-archetypes-v1"`, `authoredNavigationTopologyProfile: "authored-navigation-topology-v1"`, and the authored-node, authored-link, compiled-port, compiled-arc, and route-event capacities. It also stores `gameplayProfile: "obelisk-duel-v1"`, `enemyAiProfile: "investigative-wizard-v1"`, `enemyCapacity: 64`, `deadBodyProfile: "enemy-dead-body-v1"`, `movementSoundProfile: "proximity-walk-footsteps-v1"`, `projectileHeightCollisionProfile: "projectile-height-collision-v1"`, `elevatorProjectileCollisionProfile: "elevator-projectile-collision-v1"`, `breakawayFloorProfile: "breakaway-floor-v1"`, the current authoring-map projection, both body capacities, the sound-event capacity, and the spell baseline. Replay reconstructs independent encounter schedules, enemy homes, movement cadence, attacks, hearing, AI, dynamic-body physics, vertical support/traversal, settlement, and FIFO overwrites exactly; autonomous decisions and transitions are not synthetic input commands. Schema v21 preserves overdue sentry catch-up, schema v20 preserves unconditional independent schedules, schema v19 uses stable enemy homes with the legacy singleton encounter scheduler; schema v18 retains height-aware projectile contacts with legacy enemy homes and encounters. Schema v17 retains the same navigation behavior without projectile-height contacts; schema v16 retains the same navigation behavior with witnessed-hole pursuit disabled; schema v15 retains the same navigation behavior while forcing wizard encounters; schema v14 retains the same behavior with topology disabled; schema v13 additionally omits the breakaway profile; schema v12 retains the same behavior except for the elevator collision profile; schema v11 retains the same AI/body/sound behavior without the jump edge; schema v10 retains AI and bodies but forces full-speed silent movement and direct Fireball hearing, schema v9 retains investigative AI with corpse-free immediate removal, schema v8 selects the frozen `perceptive-wizard-v1`, schema v7 selects the frozen `tactical-wizard-v1` with its historical four-entry enemy pool, schema v6 selects frozen `basic-wizard-v1`, schema v5 keeps versioned Fireballs with frozen pre-combat behavior, and schemas v2 through v4 retain their existing legacy Fireball and particle profiles. The default projectile pool is 256 and sound queue is 257; bounded drops remain valid under excessive authored effects or deliberately reduced tuning capacities.
