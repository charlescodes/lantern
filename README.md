...
Start with the [documentation index](./docs/README.md). It separates mutable [soft specifications](./docs/soft-specs/README.md) from the durable [platform contract](./docs/platform.md), chronological milestone contracts, and regression notes. Release `0.9.0` remains the [Fireball Investigation AI](./docs/milestones/0.9.0-fireball-investigation-ai.md) boundary. The current non-release [enemy dead-body checkpoint](./docs/notes/enemy-dead-body-lifecycle.md) advances snapshots and recordings to schema v10 and performance reports to v3 without changing the application/package version. Scenario JSON remains v3, map v1, and Fireball definition v1. Schemas v2-v9 retain their frozen behavior; in particular, schema v9 keeps investigative AI with immediate enemy removal and no bodies.
...
Schema-v10 navigation keeps the schema-v9 four pinned future actor-target slots and 64 shared goal-cell fields, one preallocated builder, stable slot order, and a global 2,048-expansion tick budget. A completed field is retained while its replacement builds, but movement uses direct wall-sliding until the requested field is current. The preallocated map-cell broadphase also indexes dynamic dead bodies for deterministic collision and projectile queries. Schema-v9 investigative behavior remains frozen without bodies; schema-v8 scheduling and `perceptive-wizard-v1` replay remain frozen; schema-v7 keeps the omniscient `tactical-wizard-v1`, and schema-v6 keeps `basic-wizard-v1` and its intentional lack of pathfinding, strafing, leading, dodging, or healing retreat.
...
The developer pool instruments report dynamic/inert body occupancy and settlement counters. Render Lab reports backend, CSS/backing resolution, effective DPR, active/resident lights, frame and CPU timing, TrueSight geometry/mask/timing, and GPU timestamp availability. **Capture 10 seconds** resets timing histories without scripting gameplay. Performance-report v3 retains the v2 TrueSight fields and adds dynamic/inert body capacities, maximum occupancy, forced settlements, and FIFO overwrites to the browser/device, workload, presentation, light, spike, and GPU data.

## Snapshots and recordings

Snapshots, runtime metrics, and command recordings use schema v10. Scenario JSON uses v3. Enemy snapshots retain the schema-v9 perception, investigation, navigation, tactical, and combat record. Dead-body snapshots add ordered dynamic and inert collections with phase, identity, transform, facing, age, settlement, and compact physics fields. A 128-entry perception-event ring and bounded counters expose observations, heard explosions, accepted redirects, deduplication, and priority rejection alongside the prior events. The compact `spells` table still includes only current and referenced immutable revisions; effects retain spell code, definition revision, effect ID, and effect seed.

A live v10 recording stores `gameplayProfile: "obelisk-duel-v1"`, `enemyAiProfile: "investigative-wizard-v1"`, `enemyCapacity: 64`, `encounterMaximumAlive: 4`, `deadBodyProfile: "enemy-dead-body-v1"`, and both body capacities alongside the spell baseline. Replay reconstructs AI, dynamic-body physics, settlement, and FIFO overwrites exactly; autonomous decisions and body transitions are not synthetic input commands. Schema v9 retains investigative AI with corpse-free immediate removal, schema v8 selects the frozen `perceptive-wizard-v1`, schema v7 selects the frozen `tactical-wizard-v1` with its historical four-entry enemy pool, schema v6 selects frozen `basic-wizard-v1`, schema v5 keeps versioned Fireballs with frozen pre-combat behavior, and schemas v2 through v4 retain their existing legacy Fireball and particle profiles. The default projectile pool is 256; bounded drops remain valid under excessive authored effects.
