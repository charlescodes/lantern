...
runtime extends it through recording schema v25 and authoring-map v9 with
authored navigation, cross-floor enemy routes, independent obelisks,
destructible encounters, and authored mechanisms. Lantern combines replay-safe Fireball combat,
...
authoring-map v9, and [editable, floor-aware obelisks](./docs/plans/editable-floor-aware-obelisks.md)
...
- Press `E` to interact with a nearby lever or wall chain; face it by moving toward it. Key repeat does not retrigger it.
...
- Press LMB to cast the selected spell; Fireball is the only handler. Fireballs explode on the first opposing actor, dynamic enemy body, wall, rock, obelisk, or elevator deck/shaft they hit and pass through same-team living actors.
...
- Press `Space` to make one committed jump from a supported floor or elevator. Jump direction follows the mouse/world target; ordinary movement input does not steer the jump arc. Falling after a missed landing still permits the separate M1B.2 air control.
- While the toolbox is open, press `P` to pause, `.` to pause and advance one tick, `R` to reset the current seed, and `Shift+R` to choose a new seed.
- While the toolbox is open, press `Shift+E` to enter the paused scenario editor. Leaving edit mode restores the previous paused/running state; `F` recenters its free camera on the player.
...
Try `?arena=mechanisms` for the compact controls-and-gates regression map or
`?arena=mechanisms-pass-b` for triggered elevators, movers, and traps. The
[mechanism contract](./docs/notes/authored-mechanisms.md) explains authoring-map
v9, recording v25, logic timing, wiring, devices, and the pending browser acceptance pass.

Press `;`, then `Shift+E` to open the generated map palette, inspector, and Layers panel. Its catalog currently provides stone, moss, and single-cell hole surfaces, walls, three existing movable-rock sizes, a blocking pillar, a standing torch, a pushable table, and a two-stop elevator connector. Paint definitions drag across cells; stamp definitions place once. LMB applies the selection and RMB or **Erase** follows the active channel. Invalid placements over a wall, the player, an enemy, or another authored/active body are rejected. Layers can be created above/below, renamed, assigned signed base heights, and viewed through one non-editable reference overlay. **Shared map extent** resizes every floor from the common `(0, 0)` origin up to `128×128` cells. Expansion preserves existing coordinates and fills default floor; confirmed shrinking crops the positive X/Z edges as one undoable edit and refuses to crop the player start.

New saves emit `lantern-authoring-map` v9 with metadata, an explicit player-start layer, independently compiled named floors, separate surface/structure grids, stable layer/instance IDs, signed base heights, independent floor apertures, ordinary floor-owned obelisk/enemy/mechanism instances, one map-level connector collection, bounded authored navigation, and a typed mechanism graph. Elevators default to the historical autonomous shuttle but can instead be trigger-controlled through their graph node. Newly placed or moved elevators snap to cell centers; migrated off-center v5 elevators retain their exact deck coordinates until moved. Earlier authoring maps, legacy map v1, and scenario v2/v3 JSON migrate explicitly and resave in v9. Entering edit pauses the live simulation on the player’s current floor; authoring changes recompile and reconcile authored content, while **Restore positions**, reset, and load/import explicitly reconstruct authored body/elevator/mechanism starts. Save exports authored transforms, so runtime pushing, falling, elevator travel, mover travel, and body layer transitions never rewrite them.

For the focused M1B fixture, start `?arena=elevator` (or `?arena=elevator&renderer=3d`). For the full traversal playground, start `?arena=holes`. Its three staggered autonomous lifts connect four floors alongside holes, jumping, pressure plates, a fitting torch/rock, and an oversized table. Start `?arena=navigation` to test authored cross-floor routes against low-damage Urchins instead of Fireball-casting wizards. The lift carries any supported body without centering or locking X/Z control; it keeps cycling whether empty or occupied. The console probes `__lantern.elevators()`, `__lantern.verticalBody(kind, id)`, `__lantern.cycleElevator(id)`, and `__lantern.summonElevator(id, stop)` expose deterministic diagnostics.
...
Player and enemy wizards each have a `0.3m` radius, `75kg` mass, the same movement fundamentals, and `100` maximum health. Health regenerates at `1 HP/s` after five damage-free seconds. Direct opposing Fireball hits deal `25`; splash deals `25 × clamp(1 - surfaceDistance / capturedBlastRadius, 0, 1)`. A current obelisk has `600` non-regenerating health, takes opposing combat damage through the same event path, and stops spawning when destroyed. Its authored solid/occluding cell then opens and both renderers leave walkable rubble until reset restores the pristine authored state. Solid walls and living obelisks block splash. Casters and allies are immune to health damage, while the existing blast impulse remains team-neutral.
...
The opt-in 3D route renders a floor, 2.5m instanced wall cells, bounded pristine/rubble obelisk groups, preallocated player/enemy silhouettes and health tracks, low-poly rocks, chest-height fireballs, and one instanced spark mesh using existing particle `x/y/z` and `currentSize` values. All 64 enemy instances, 64 front-facing hood/nose markers, 129 `0.10m × 0.90m` actor/obelisk health tracks and fills, and the configured 116 default dead-body instances exist before renderer warmup. Dead bodies reuse the cylinder geometry with a darker material and a shared fall pose; Canvas2D renders the same pose as an oriented capsule. Both share TrueSight concealment and add no light. The default light pool still contains 16 resident, shadowless point lights: two atomic eight-slot effect groups.
...
Snapshots, runtime metrics, and command recordings use schema v25. Scenario JSON uses v3 and current recordings retain an initial authoring-map v9 document alongside its frozen scenario-v3 compatibility projection. Schema v25 records `authored-mechanisms-v2`: trigger-controlled elevators, safe kinematic movers, environmental bolts/Fireballs, floor spikes, and wall spears. Schema v24 remains frozen at `authored-mechanisms-v1` and authoring-map v8. Schema v23 records `destructible-obelisks-v1`: each runtime obelisk starts at 600 health, emits ordinary damage/death events, stops spawning at zero, and removes only its disposable runtime collision/occlusion cell until reset. Schema v22 records `banked-sentry-obelisk-encounters-v3`; schemas v2-v22 retain invulnerable obelisks. Schema v21 preserves `sentry-obelisk-encounters-v2`, including its historical overdue catch-up cadence and the same range/visibility gate. Schema v20 preserves unconditional independent obelisk encounters; schema v19 records each enemy's floor-aware home identity and return routing; schema v18 records height-aware projectile contacts. Schema v17 records `observed-hole-pursuit-v1`: pursuit-capable enemies may follow a directly witnessed player drop through a live hole, without learning an unseen landing position; schemas v2-v16 retain their exact prior behavior. Schema v16 records `enemy-archetypes-v1` and the encounter's `wizard` or `urchin` selection; schemas v2-v15 remain wizard-only. Schema v15 records the inert `authored-navigation-topology-v1` profile and bounded capacities; schemas v2-v14 preserve their prior AI behavior with topology disabled. Schema v14 records `breakaway-floor-v1`: an 18-tick latched floor-to-hole transition that reuses ordinary aperture/falling authority. Schema v13 records the `elevator-projectile-collision-v1` profile: Fireballs collide with a stopped deck on either endpoint and the lower-floor piston/shaft while it is away from its lower stop. Schema v12 retains the committed jump edge and its pre-elevator-projectile collision behavior. Player snapshots include movement, vertical/support, layer, aperture, and jump state; enemies and their bodies retain archetype, physical identity, and live transform state. The one-tick sound queue and newest 32 of a separate 128-entry diagnostic ring expose stable source/event identity, origin, radius, and bounded occupancy/drop/hearing counters. The compact `spells` table still includes only current and referenced immutable revisions; effects retain spell code, definition revision, effect ID, and effect seed.

The v23 compatibility boundary introduced `obeliskDestructionProfile: "destructible-obelisks-v1"` beside `obeliskEncounterProfile: "banked-sentry-obelisk-encounters-v3"`, `enemyHomeProfile: "floor-aware-enemy-home-v1"`, `holePursuitProfile: "observed-hole-pursuit-v1"`, `enemyArchetypeProfile: "enemy-archetypes-v1"`, `authoredNavigationTopologyProfile: "authored-navigation-topology-v1"`, and the existing bounded configuration. Replay reconstructs obelisk health, destruction, encounter shutdown, and the opened runtime cell exactly. Schema v22 and earlier force obelisk destruction off; their other frozen profile boundaries remain unchanged. The default projectile pool is 256 and sound queue is 257; bounded drops remain valid under excessive authored effects or deliberately reduced tuning capacities.
