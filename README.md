...
For the focused M1B fixture, start `?arena=elevator` (or `?arena=elevator&renderer=3d`). For the full traversal playground, start `?arena=holes`. Its three staggered autonomous lifts connect four floors alongside holes, jumping, pressure plates, a fitting torch/rock, and an oversized table. The lift carries any supported body without centering or locking X/Z control; it keeps cycling whether empty or occupied. The console probes `__lantern.elevators()`, `__lantern.verticalBody(kind, id)`, `__lantern.cycleElevator(id)`, and `__lantern.summonElevator(id, stop)` expose deterministic diagnostics.
