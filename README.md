...
runtime extends it through recording schema v24 and authoring-map v8 with
...
authoring-map v8, and [editable, floor-aware obelisks](./docs/plans/editable-floor-aware-obelisks.md)
...
Try `?arena=mechanisms` for the controls-and-gates acceptance map. The
[mechanism contract](./docs/notes/authored-mechanisms.md) explains authoring-map
v8, recording v24, logic timing, wiring, and the pending browser acceptance pass.
...
New saves emit `lantern-authoring-map` v8 with metadata, an explicit player-start layer, independently compiled named floors, separate surface/structure grids, stable layer/instance IDs, signed base heights, independent floor apertures, ordinary floor-owned obelisk/enemy instances, one map-level connector collection, and bounded authored navigation nodes/links. Elevators are autonomous: each stop dwells for one second and each leg takes two seconds by default, regardless of floor-height gap. Newly placed or moved elevators snap to cell centers; migrated off-center v5 elevators retain their exact deck coordinates until moved. Earlier authoring maps, legacy map v1, and scenario v2/v3 JSON migrate explicitly and resave in v8. Entering edit pauses the live simulation on the player’s current floor; authoring changes recompile and reconcile authored content, while **Restore positions**, reset, and load/import explicitly reconstruct authored body/elevator starts. Save exports authored transforms, so runtime pushing, falling, elevator travel, and body layer transitions never rewrite them.
