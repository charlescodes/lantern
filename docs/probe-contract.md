...
| AI and visibility | `encounterDiagnostics`, `enemyDiagnostics`, `aiView`, `trueSight`, `isVisible`, `navigationTopology`, `navigationRouteEvents` | `setAiView`, `removeEntity` for living enemies | Runtime enemy despawn is a fixed-tick, replay-visible debug action; an owning obelisk may replace the enemy on a later spawn attempt. Topology reads are detached and bounded; AI View and TrueSight calls are diagnostic/presentation controls and do not grant simulation knowledge. |
