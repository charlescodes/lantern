...
test("reset preserves the applied revision and makes it the schema-v9 recording baseline", () => {
  const simulation = new Simulation();
  const changed = definition((value) => {
...
test("schema-v9 recording replays definitions, revisions, effects, and explicit seeds exactly", () => {
  const authored = definition((value) => {
    value.cast.cooldown = 0;
