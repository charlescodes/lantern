...
test("snapshot, runtime, and recording use the current schema while scenarios remain v3", () => {
  const simulation = new Simulation({
  ...
  assert.equal(snapshot.schemaVersion, 25);
  assert.equal(runtime.metrics().schemaVersion, 25);
...
test("schema-4 recordings capture initial particle profile and collision modes", () => {
  const simulation = new Simulation({
    map: borderedMap(24, 24, { x: 3.5, z: 18.5 }),
...
test("schema-2 recordings replay with legacy non-colliding particles", () => {
  ...
  legacyRecording.initialScenario.version = 2;
